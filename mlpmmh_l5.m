...
Nl =  floor(0.5 * 2^(2*LP) * hl*(KL+5)+ 100);
...
function [lwf, lwc, cz] = c_pf_db(L, NP, T, T_obs, X,a1,a2,a3,a4,sig1,sig2,rho)
    ...
    steps_E = T_obs(T+1)-T_obs(T);
    nlfE = ceil(steps_E/hlf);
    nlcE = ceil(steps_E/hlc);
    
    [samp_fe, logf_dene] = h_log_normal(NP, steps_E,a1,a2,a3,a4,sig1,sig2, rho, XKMf_E(:,1), XKMf_E(:,2),  -1, -1);
    for n = 1:NP 
        Xf_est(n,1,T+1) = samp_fe(1,n);
        Xf_est(n,2,T+1) = samp_fe(2,n);
        lGLf2(n,T) = logf_dene(n);
    end
    XKf_E(:,1) = Xf_est(:,1,T+1);
    XKf_E(:,2) = Xf_est(:,2,T+1);

    [logf_densitye,~] = linear_tran_t_aux(NP, 0, steps_E, a1,a2,a3,a4,sig1,sig2,rho, XKMf_E(:,1), XKMf_E(:,2), XKf_E(:,1), XKf_E(:,2),XKMf_E(:,1), XKMf_E(:,2));
    for n = 1:NP 
        lGLf1(n,T) = logf_densitye(n);
    end

    [samp_ce, logc_dene] = h_log_normal(NP, steps_E,a1,a2,a3,a4,sig1,sig2, rho, XKMc_E(:,1), XKMc_E(:,2),-1, -1);
    
    for n = 1:NP 
        Xc_est(n,1,T+1) = samp_ce(1,n);
        Xc_est(n,2,T+1) = samp_ce(2,n);
        lGLc2(n,T) = logc_dene(n);
    end
    XKc_E(:,1) = Xc_est(:,1,T+1);
    XKc_E(:,2) = Xc_est(:,2,T+1);
    
    [logc_densitye,~] = linear_tran_t_aux(NP, 0, steps_E, a1,a2,a3,a4,sig1,sig2,rho, XKMc_E(:,1), XKMc_E(:,2), XKc_E(:,1), XKc_E(:,2),XKMc_E(:,1), XKMc_E(:,2));
    for n = 1:NP 
        lGLc1(n,T) = logc_densitye(n);
    end
    
    [Xf_Brie, Xc_Brie, derif_logxe, deric_logxe] = cou_em_linear(XKMf_E, XKf_E, XKMc_E,XKc_E,steps_E, L, NP, a1,a2,a3,a4,sig1,sig2,rho);         
    Rfce = Xf_Brie(:, 1,1);
    Ffce = Xf_Brie(:, 2,1);
    Rfbe = Xf_Brie(:, 1,1);
    Ffbe = Xf_Brie(:, 2,1);
    ...
    for m = 0:nlfE-2 
        dif_mufe = zeros(NP,2);
        tfce = m*hlf;
        tf1e = T_obs(T+1) - T_obs(T);
        for n = 1:NP
            dif_mufe(n,:) = [a2*Rfce(n)*(Ffbe(n)- Ffce(n) + tfce/tf1e*(Ffee(n)-Ffbe(n))), a3*Ffce(n)*(Rfce(n)- Rfbe(n)+ tfce/tf1e*(Rfbe(n)-Rfee(n)))];
            drif_fe = derif_logxe(n,:,m+1);
            lGLf3(n,T) = lGLf3(n,T) + hlf*(dif_mufe(n,:)*drif_fe');
        end
        if m ~= nlfE-2
            Rfce = Xf_Brie(:,1,m+2);
            Ffce = Xf_Brie(:,2,m+2);
    ...
    for m2 = 0:nlcE-2 
        dif_muce = zeros(NP,2);
        tcce = m2*hlc;
        tc1e = T_obs(T+1) - T_obs(T);
        for n = 1:NP
            dif_muce(n,:) = [a2*Rcce(n)*(Fcbe(n)- Fcce(n) + tcce/tc1e*(Fcee(n)-Fcbe(n))), a3*Fcce(n)*(Rcce(n)- Rcbe(n)+ tcce/tc1e*(Rcbe(n)-Rcee(n)))];
            drif_ce= deric_logxe(n,:,m2+1);
            lGLc3(n,T) = lGLc3(n,T) + hlc*(dif_muce(n,:)*drif_ce');
        end
    ...
    lGLJ(:,T) = log(0.5*exp(lGLf(:,T)) + 0.5*exp(lGLc(:,T)));


    GL0E = exp(lGLJ(:,T) - max(lGLJ(:,T)));
    lGL_star(1,T)= log(sum(GL0E)) + max(lGLJ(:,T));

    lwf = lwf + mean((lGLf(:,T) - lGLJ(:,T)));
    lwc = lwc + mean((lGLc(:,T) - lGLJ(:,T)));
