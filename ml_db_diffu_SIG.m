%% MLPMCMC with diffusion bridge, estimating the diffusion coefficient
%EM observation regular bserved
...
%Theta_A = [1.02,0.26,-0.28,0.7];
%Theta_A = [0.9,0.2,-0.3,0.6];
Theta_A = [-0.1, -0.1, 0.5];
% sigma1 and sigma2 should always be positive
tic;
%PMMH at L = Lmin
Theta_A_p = Theta_A;
% sigma1 and sigma2 should always be positive
Theta_SIG_p = [exp(Theta_A_p(1)),exp(Theta_A_p(2)), 2/(1+exp(-Theta_A_p(3))) -1];

Z = PF_DB(Lmin, NP, T, X, A(1,1), A(1,2), A(2,1),A(2,2), Theta_SIG_p(1),Theta_SIG_p(2),Theta_SIG_p(3));
lZ = Z;
l_pos_Theta_A_p = l_posterior(Theta_A_p, lZ);

N_count = 0;
N_count_last = 0;
%Sigma_A = 0.3*diag([0.8,0.6,0.6,0.8]);
Sigma_A = 0.5*diag([0.6,0.6,0.9]);
Sigma_A1 = 0.35*diag([0.6,0.6,0.9]);
for iter = 1:Nl(1)
    ...
    Theta_SIG_prime = [exp(Theta_A_prime(1)), exp(Theta_A_prime(2)), 2/(1+exp(-Theta_A_prime(3)))-1];

    Z_prime = PF_DB(Lmin, NP, T, X, ...
            A(1,1),A(1,2),A(2,1),A(2,2), Theta_SIG_prime(1),Theta_SIG_prime(2),Theta_SIG_prime(3));
    lZ_prime = Z_prime;
    ...
    if U < alpha
        ...
        lZ = PF_DB(Lmin, NP, T, X, A(1,1),A(1,2),A(2,1),A(2,2), Theta_SIG_p(1),Theta_SIG_p(2),Theta_SIG_p(3));
        l_pos_Theta_A_p = l_posterior(Theta_A_p, lZ);
    end 
end
...
for l = 1:LP - Lmin 
    ...
    Theta_SIG_l = [exp(Theta_l(1)),exp(Theta_l(2)),2/(1+exp(-Theta_l(3)))-1];
    
    %Theta_l = Theta_A;
    %Theta_l = [0.909931112401148   0.731373870754993  -0.430673287516278   1.062162701066439];
    [H1_l, H2_l, G_l] = c_pf_db(level, NP_1, T, X, A(1,1),A(1,2),A(2,1),A(2,2), Theta_SIG_l(1),Theta_SIG_l(2),Theta_SIG_l(3));
    lG_l = G_l;
    ...
    for iter = 1:Nl(l+1)
        ...
        Theta_SIG_l_prime = [exp(Theta_l_prime(1)),exp(Theta_l_prime(2)), 2/(1+exp(-Theta_l_prime(3)))-1];
        
        [H1_lp, H2_lp, lG_lp] = c_pf_db(level, NP_1, T, X, A(1,1),A(1,2),A(2,1),A(2,2), Theta_SIG_l_prime(1),Theta_SIG_l_prime(2),Theta_SIG_l_prime(3));
        %lG_lp = G_lp;
        ...
        if Ul < alpha_l 
            ...
            [H1_l, H2_l, lG_l] = c_pf_db(level, NP_1, T, X, A(1,1),A(1,2),A(2,1),A(2,2), Theta_SIG_l(1),Theta_SIG_l(2),Theta_SIG_l(3));
            l_pos_theta_l = l_posterior(Theta_l, lG_l);
        ...
        %{
        for i = 1:4      
            Theta_trace_1{l,1}(:,i) = Theta_trace{l+1,1}(:,i) .* (exp(H1_trace{l,1}(:,1)) / mean(exp(H1_trace{l,1}(:,1))));
            Theta_trace_2{l,1}(:,i) = Theta_trace{l+1,1}(:,i) .* (exp(H2_trace{l,1}(:,1)) / mean(exp(H2_trace{l,1}(:,1))));
            Theta_trace{l+1,1}(:,i) = Theta_trace{l+1,1}(:,i) .* (exp(H1_trace{l,1}(:,1)) / mean(exp(H1_trace{l,1}(:,1))) - exp(H2_trace{l,1}(:,1))/ mean(exp(H2_trace{l,1}(:,1))));
            ML_Theta_trace{l+1,1}(:,i) = cumsum(Theta_trace{l+1, 1}(:,i)) ./ (1:Nl(l+1))';
        end
        %}
...
function lpos_p = l_posterior(Theta, lik)
    ...
    log_prior = lG(Theta(1),0,1) + lG(Theta(2),0,1) + lG(Theta(3),0,1);
